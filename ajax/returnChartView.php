...
if (isset($_SESSION["user_id"])) {
    ...
    $k = 1;
    for ($i = 11; $i >= 2; $i--) {
        $query = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 and unix_time>'" . ${"sevenDays$i"} . "' and unix_time<='" . ${"sevenDays$j"} . "'")->fetch_assoc();
        $Transaction[$k] = number_format($query["TOPLAM"],"2",".","");

        $query = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>'" . ${"sevenDays$i"} . "' and unixtime<='" . ${"sevenDays$j"} . "' and status=1")->fetch_assoc();
        $Deposit[$k] = number_format($query["TOPLAM"],"2",".","");
        $j--;
        $k++;
    }
    ...
    $query1 = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>='".$ThisMonthStart."' and status=1")->fetch_assoc();
    if($query1["TOPLAM"]==""){
        $query1["TOPLAM"] = 0;
    }
    $query2 = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>='".$LastMonthStart."' and unixtime<'".$ThisMonthStart."' and status=1")->fetch_assoc();
    if($query2["TOPLAM"]==""){
        $query2["TOPLAM"] = 0;
    }

    $query3 = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 and unix_time>='".$ThisMonthStart."'")->fetch_assoc();
    if($query3["TOPLAM"]==""){
        $query3["TOPLAM"] = 0;
    }
    $query4 = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 and unix_time>='".$LastMonthStart."' and unix_time<'".$ThisMonthStart."'")->fetch_assoc();
    if($query4["TOPLAM"]==""){
        $query4["TOPLAM"] = 0;
    }

    $rows["ChartTotalDeposit"] = number_format($query1["TOPLAM"],"2",".","")." TL";
    ...
    $rows["ChartTotalDepositSpan"] = $HarcamaOran;
