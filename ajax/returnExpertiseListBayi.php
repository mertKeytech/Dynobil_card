...
if (isset($_SESSION["user_id"])) {
    ...
    WHERE transactions.transaction_status=4 and transactions.distributor_id='".$_SESSION["distributor_id"]."' ORDER BY transactions.unix_time DESC LIMIT 20 ");
    $i = 0;
    while($row = $Expertiz->fetch_assoc()){
        ...
        if($row["ID"]==""){
            $Button[$i] .= "<a href='index.php?module=sales/upload_expertise&do=upload_file_list&id=".$row["TransID"]."'><button type='button' class='btn btn-success'>RAPORU YÜKLE</button></a>";
        } else {
            $Button[$i] .= "<a href='index.php?module=sales/upload_expertise&do=upload_file_list&id=".$row["TransID"]."'><button type='button' class='btn btn-info'>RAPORLARA GÖZAT</button></a>";
        }
        $Button[$i] .= '</div></div></li>';
        $i++;
    }
