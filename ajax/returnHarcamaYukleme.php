...
if (isset($_SESSION["user_id"])) {
    ...
    //END
    //Harcama Haftalık
    $HarcamaTRY = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>='".strtotime($week)."'")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    $HarcamaTRYGecen = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>='".strtotime($lastweek)."' and unix_time<='".strtotime($week)."'")->fetch_assoc();
    ...
    //END
    //Harcama Aylık
    $HarcamaTRY = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>'".$lastMonthEnd."'")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    $HarcamaTRYGecen = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>='".$lastMonthStart."' and unix_time<='".$lastMonthEnd."'")->fetch_assoc();
    if($HarcamaTRYGecen["TOPLAM"]==""){
        $HarcamaTRYGecen["TOPLAM"] = 1;
    }
    ...
    //END
    // TOPLAM Harcama
    $HarcamaTRY = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    ...
    //END
    //Yükleme Adet
    $HarcamaAdet = $db->query("SELECT * FROM deposits WHERE unixtime>='".$strToday."' and status=1")->num_rows;
    $HarcamaAdetGecen = $db->query("SELECT * FROM deposits WHERE unixtime>='".$strSeven."' and unixtime<='".$strSevenEnd."' and status=1")->num_rows;
    ...
    //END
    //Yükleme Haftalık
    $HarcamaTRY = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>='".strtotime($week)."' and status=1")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    $HarcamaTRYGecen = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>='".strtotime($lastweek)."' and unixtime<='".strtotime($week)."' and status=1")->fetch_assoc();
    if($HarcamaTRYGecen["TOPLAM"]==""){
        $HarcamaTRYGecen["TOPLAM"] = 1;
    }
    ...
    //END
    //Yükleme Aylık
    $HarcamaTRY = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>'".$lastMonthEnd."' and status=1")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    $HarcamaTRYGecen = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE unixtime>='".$lastMonthStart."' and unixtime<='".$lastMonthEnd."' and status=1")->fetch_assoc();
    if($HarcamaTRYGecen["TOPLAM"]==""){
        $HarcamaTRYGecen["TOPLAM"] = 1;
    }
    ...
    //END
    // TOPLAM Yükleme
    $HarcamaTRY = $db->query("SELECT SUM(amount) AS TOPLAM FROM deposits WHERE status=1")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
