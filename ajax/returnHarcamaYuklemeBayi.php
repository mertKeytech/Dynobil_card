...
if (isset($_SESSION["user_id"])) {
    ...
    //END
    //Harcama Haftalık
    $HarcamaTRY = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>='".strtotime($week)."' and distributor_id='".$_SESSION["distributor_id"]."'")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    $HarcamaTRYGecen = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>='".strtotime($lastweek)."' and unix_time<='".strtotime($week)."' and distributor_id='".$_SESSION["distributor_id"]."'")->fetch_assoc();
    ...
    //END
    //Harcama Aylık
    $HarcamaTRY = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>'".$lastMonthEnd."' and distributor_id='".$_SESSION["distributor_id"]."'")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
    $HarcamaTRYGecen = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 AND unix_time>='".$lastMonthStart."' and unix_time<='".$lastMonthEnd."' and distributor_id='".$_SESSION["distributor_id"]."'")->fetch_assoc();
    if($HarcamaTRYGecen["TOPLAM"]==""){
        $HarcamaTRYGecen["TOPLAM"] = 1;
    }
    ...
    //END
    // TOPLAM Harcama
    $HarcamaTRY = $db->query("SELECT SUM(amount-campaign_amount) AS TOPLAM FROM transactions WHERE transaction_status=4 and distributor_id='".$_SESSION["distributor_id"]."'")->fetch_assoc();
    if($HarcamaTRY["TOPLAM"]==""){
        $HarcamaTRY["TOPLAM"] = 0;
    }
